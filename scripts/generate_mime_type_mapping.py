...
def render_cpp(entries: Iterable[Tuple[str, str]], mapping: MappingDirection) -> str:
    ...
    lines.extend(
        [
            ...
            f"    const auto entry_slice = Slice(entry.{mapping.key_field});\n",
            "    const auto min_size = std::min(entry_slice.size(), value.size());\n",
            "    const auto diff = std::memcmp(entry_slice.data(), value.data(), min_size);\n",
            "    if (diff != 0) {\n",
            "      return diff < 0;\n",
            "    }\n",
            "    return entry_slice.size() < value.size();\n",
            "  };\n",
            "  const auto equal_entry = [](const MimeTypeExtensionEntry &entry, Slice value) {\n",
            f"    const auto entry_slice = Slice(entry.{mapping.key_field});\n",
            "    return entry_slice.size() == value.size() &&\n",
            "           std::memcmp(entry_slice.data(), value.data(), value.size()) == 0;\n",
            "  };\n",
            f"  auto it = std::lower_bound(begin, end, {mapping.key_field}, compare_entry);\n",
            f"  if (it != end && equal_entry(*it, {mapping.key_field})) {{\n",
